...
#             cond_A = rule["LT_A"] is None or min_LT_A >= rule["LT_A"]
#             cond_B = rule["LT_B"] is None or min_LT_B >= rule["LT_B"]
#             cond_C = rule["GT_C"] is None or max_GT_C < rule["GT_C"]
...
#         # ============================================================
#         # CONTINUOUS (ROBUST VERSION)
#         # ============================================================
        
#         def normalize(a, b, c):
#             """
#             returns position of a between b (better) and c (worse)
#             output in [0,1]
#             """
#             if b == c:
#                 return 0.0
...
#             for m, obs in metrics.items():
        
#                 ref = rule[m]
#                 if ref is None:
#                     continue
        
#                 # Tier 1: best region → higher obs is worse
#                 p = normalize(obs, 1.0, ref) if m in ["GT_C"] else normalize(obs, ref, 0.0)
#                 progresses.append(p)
#                 print("Scenario: " + scenID + ", discrete: " + str(discrete_tier) + ", obs: " + str(obs) + ", continuous: " + str(p))
        
#             progress = np.nanmean(progresses) if progresses else 0
#             continuous = 1 + 0.999 * progress
...
#                 p = normalize(obs, ref, 1.0) if m in ["GT_C"] else normalize(obs, ref, 0.0)
#                 progresses.append(p)
#                 print("Scenario: " + scenID + ", discrete: " + str(discrete_tier) + ", obs: " + str(obs) + ", continuous: " + str(p))
        
#             progress = np.nanmean(progresses) if progresses else 0
#             continuous = 4 + 0.999 * progress
        
        
#         # -------------------------
#         # Middle tiers
#         # -------------------------
#         # PROBLEM: DISCRETE TIER 3 GETS ALWAYS MAPPED TO 3.999, AND I SUSPECT THE SAME WOULD 
#         else:
#             # THE ISSUE IS HERE:
#             lower = tier_rules[discrete_tier]
#             upper = tier_rules[discrete_tier + 1]
#             # THIS MAKES MORE SENSE, BETTER IS THE LOWER TIER BOUNDARY
#             better = tier_rules[discrete_tier - 1]
#             current = tier_rules[discrete_tier]        
#             progresses = []
...
#                 # Switch normalization as hi is worse?
#                 # p = normalize(obs, lo, hi)
#                 p = normalize(obs, hi, lo)
...
#             continuous = discrete_tier + 0.999 * progress        
